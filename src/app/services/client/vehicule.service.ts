...
export class VehiculeService {
  ...
  getModeles(): Observable<{data: any[], message: string}> {
    const headers = this.authService.getAuthHeaders().set('Content-Type', 'application/json');
    return this.http.get<{data: any[], message: string}>(`${this.apiUrl}/modele/tous`, { headers });
  }


  addVehiculeToUtilisateur(body: any): Observable<{ data: any[], message: string }> {
    const headers = this.authService.getAuthHeaders().set('Content-Type', 'application/json');

    return this.http.post<{ data: any[], message: string }>(
        `${this.apiUrl}/vehicule/ajout_utilisateur`,
        body,
        { headers }
    );
  }
